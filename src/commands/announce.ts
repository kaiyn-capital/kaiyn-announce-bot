...
const announceCommand: BotCommand = {
	data: new SlashCommandBuilder()
		...
		.addStringOption((option) =>
			option.setName("image").setDescription("大圖 URL").setRequired(false),
		)
		.addStringOption((option) =>
			option.setName("thumbnail").setDescription("縮圖 URL").setRequired(false),
		)
	...
	async handleModalSubmit(interaction) {
		...
		try {
			...
			if (
				!announcement ||
				isPendingExpired(announcement.expiresAt, Date.now())
			) {
				pendingAnnouncements.delete(modalId);
				await replyEphemeral(
					interaction,
					"公告表單已過期，請重新使用 /announce。",
				);
				return true;
			}
